...
export default function GameHeader({ game, players, currentTurn, onPass }) {
  return (
    <div className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-white/80 px-5 py-4 shadow-sm">
      <div className="flex items-center gap-3">
        <span
          className={`h-3 w-3 rounded-full ${COLOR_STYLES[currentTurn?.color] || 'bg-slate-300'}`}
        />
        <div>
          <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Current Turn</p>
      ...
      <div className="flex items-center gap-3">
        <div className="rounded-full border border-dashed border-slate-300 px-4 py-2 text-xs font-semibold tracking-[0.3em] text-slate-700">
          {game?.room_code || '------'}
        </div>
        <button
          type="button"
